...
class JavaScriptAnalyzer(BaseAnalyzer):
    ...
    _CONTROL_KEYWORDS = (
        ...
        'return', 'throw', 'typeof', 'instanceof', 'new', 'delete', 'in', 'of',
        'await', 'yield', 'with', 'void', 'super',
    )
    ...
    INTERFACE_PATTERN = re.compile(r'^\s*(?:export\s+)?(?:declare\s+)?interface\s+([A-Za-z_$][\w$]*)', re.MULTILINE)
    TYPE_ALIAS_PATTERN = re.compile(r'^\s*(?:export\s+)?(?:declare\s+)?type\s+([A-Za-z_$][\w$]*)\s*[=<]', re.MULTILINE)
    ENUM_PATTERN = re.compile(r'^\s*(?:export\s+)?(?:const\s+|declare\s+)?enum\s+([A-Za-z_$][\w$]*)', re.MULTILINE)
